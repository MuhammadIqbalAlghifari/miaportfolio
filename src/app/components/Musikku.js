...
import { Musikku } from "../libs/ProjectsData"
import { Text } from "@chakra-ui/react";
import { useEffect, useRef } from "react";
import gsap from "gsap";
import { easeOut } from "framer-motion";

export default function MusikkuProject() {

  const musikkuContainer = useRef(null)

  let TextItem, ButtonItem = useRef(null)

  useEffect(() => {
    gsap.fromTo(musikkuContainer.current.children, 
      {
        opacity: 0,
      },
      {
          ...
          <div className="w-full absolute bg-no-repeat -z-10 bg-cover">
            ...
            <section className="px-10 transition-all duration-500 relative" style={{fontFamily: 'Futura Hv', zIndex: 1}}>
                  <div className="flex flex-col max-w-7xl mx-auto backdrop-blur-md rounded-lg md:mt-24 duration-500 mt-20 p-8 hover:bg-black hover:bg-opacity-50 duration justify-center h-full items-center py-4">
                    ...
                    <div ref={musikkuContainer}>
                      {Musikku.map(({Name, Description, Category, Status, Language, TechStack, Date, Image1, Image2, Image3, Image4, Image5, HrefWebsite, HrefGithub, index}) => (
                        <>
                          ...
                          <div className="flex justify-center md:mt-10 mt-8 items-center gap-3 md:gap-4">
                              <button disabled href={HrefWebsite} target="_blank" rel="noopener noreferrer" className="px-4 py-2 border flex justify-center items-center md:gap-4 gap-2 border-white rounded-lg text-white hover:text-[#00da8e] hover:border-[#00da8e] lg:text-md md:text-base text-xs duration-500">
                              ...
                              <button disabled href={HrefGithub} target="_blank" rel="noopener noreferrer" className="px-4 py-2 border flex justify-center items-center md:gap-4 gap-2 border-white rounded-lg text-white hover:text-[#00da8e] hover:border-[#00da8e] lg:text-md md:text-base text-xs duration-500">
